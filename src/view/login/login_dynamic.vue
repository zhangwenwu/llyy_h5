	...
	<div class='login_dynamic'>
		...
		<div class='login_form'>
			<div class="user_name">
	 			...
	 			<input type="text" id="login_name" placeholder="请输入手机号/邮箱" ref='phone' @blur='isphone()'>
	 		</div>
	 		...
	 		<div class='dt_mi'><a>账号密码登录</a></div>
	 		</router-link>
		</div>
	</div>
</template>
	    ...
	    login:function(){                 	//登录
	        if(this.$refs.phone.value == ''){
	        	...
	        	alert('请输入验证码');
	        	 ...
	        	 this.$axios({     
		            method: 'post',
		            url: 'http://219.135.58.197:8888/llhb/m/loginByPhoneOrEmail',
		            params:{
	                  phone: this.$refs.phone.value,
	                  dynamicNum: this.$refs.yzm.value,
	                },
		         })
		        .then((response) => {
		        	if(response.data.loginErr == 'ok'){
		            	this.$router.push({path:'/personal/personal'})
		        	}else{
		        		
		        		alert(response.data.loginErr);
		        	}
		        })
	        }
	    },
